...
def preprocess_text(text):
    # Replace newline characters and clean text
    text = text.replace('\\n', ' ').replace('\\', '')
    text = text.replace('"', '')
    ...
    # Initialize the lemmatizer and lemmatize tokens
...
def preprocess_keywords(keyword_string):
    # Split keyword string into a list and clean each keyword
    keywords = [keyword.strip().lower().replace('"', '') for keyword in keyword_string.split(',')]
    return keywords

# Define a set of keywords related to mental health
...
def load_data(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            data = json.load(file)
            print(f"Loaded data from {file_path}")  # Debugging
            return data
...
if __name__ == "__main__":
    ...
    # Process the directory and filter data
